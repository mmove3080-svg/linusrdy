 ...
 */
export function StatusCard({ x, y, label, value, accent, fadeIn = false }: StatusCardProps) {
  const width = Math.max(label.length * 6.4, value.length * 7.6) + 30;
  const height = 46;
  ...
  const above = y - 18 - height > 8;
  const top = above ? y - 18 - height : y + 26;
  ...
  return (
    <g
      ...
      aria-hidden="true"
    >
      <rect x={cx - width / 2} y={top} width={width} height={height} rx={12} fill="#FFFFFF" />
      {above ? (
        <path d={`M ${x - 6} ${top + height} l 6 8 l 6 -8 Z`} fill="#FFFFFF" />
      ) : (
        <path d={`M ${x - 6} ${top} l 6 -8 l 6 8 Z`} fill="#FFFFFF" />
      )}
      <text
        x={cx - width / 2 + 15}
        y={top + 18}
